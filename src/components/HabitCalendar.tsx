...
export const HabitCalendar: React.FC<HabitCalendarProps> = ({
  ...
  const [tooltip, setTooltip] = useState<{
    ...
    y: number;
    date: string;
    items: { name: string; color: string; done: boolean }[];
  } | null>(null);

  const days = useMemo(() => getDaysOfYear(year), [year]);

  // Build a map: dateISO -> array of { habit, done, planned }
  const dayMap = useMemo(() => {
    const map = new Map<
      string,
      { habit: Habit; done: boolean; planned: boolean }[]
    >();

    days.forEach((day) => {
      const iso = toISO(day);
      const entries: { habit: Habit; done: boolean; planned: boolean }[] = [];
      habits.forEach((h) => {
        const planned = isHabitPlannedForDay(
          h.frequency,
          h.frequencyDays,
          day,
          h.startDate,
        ...
        const done = h.completedDays.includes(iso);
        if (planned || done) {
          entries.push({ habit: h, done, planned: planned && !done });
        }
      });
  ...
  const renderCell = (day: Date, idx: number) => {
    ...
    return (
      <g key={iso}>
        {entries.map((entry, i) => {
          const alpha = entry.planned ? 0.4 : 1;
          const fill = hexToRgba(entry.habit.color, alpha);
          return (
            <rect
              key={entry.habit.id}
              x={x + i * stripeWidth}
